...
- **Human readable and writeable pipelines.** We're reworking pipelines so you
  ...
  pipline `.dvc` files into a single file (yay, simpler working directory). No worries for pipeline enthusiasts- DVC 1.0 is backwards compatible, so your existing projects won't be interrupted.
